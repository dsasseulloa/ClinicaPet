...
namespace Projeto.DAO
{
    ...
    {
        ProjetoDBContext db = new ProjetoDBContext();
        ...
        public Dictionary<string, decimal?> CalculateWeeklyExpense()
        {
            ...
            var pastDate2 = DateTime.UtcNow.AddDays(-28);
            var pastDate3 = DateTime.UtcNow.AddDays(-7);
            var pastDate4 = DateTime.UtcNow.AddDays(-7);

            decimal? foodSum = db.ExpenseReport.Where
                (cat => cat.Category == "Food" && (cat.ExpenseDate > pastDate))
                .Select(cat => cat.Amount)
                .Sum();

            decimal? shoppingSum = db.ExpenseReport.Where
               (cat => cat.Category == "Shopping" && (cat.ExpenseDate > pastDate2))
               .Select(cat => cat.Amount)
               .Sum();

            decimal? travelSum = db.ExpenseReport.Where
               (cat => cat.Category == "Travel" && (cat.ExpenseDate > pastDate2))
               .Select(cat => cat.Amount)
               .Sum();

            decimal? healthSum = db.ExpenseReport.Where
               (cat => cat.Category == "Health" && (cat.ExpenseDate > pastDate2))
               .Select(cat => cat.Amount)
               .Sum();
